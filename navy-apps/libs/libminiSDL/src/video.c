...
void SDL_BlitSurface(SDL_Surface *src, SDL_Rect *srcrect, SDL_Surface *dst, SDL_Rect *dstrect) {
  ...
  int dw,dh,dx,dy,sw,sh,sx,sy;
  uint32_t ww = (dst->format->palette!=NULL)? 1 : 4;
  if(dstrect==NULL){dw=dst->w;dh=dst->h;dx=0;dy=0;}
  else {dw=dstrect->w;dh=dstrect->h;dx=dstrect->x;dy=dstrect->y;}
  if(srcrect==NULL){sw=src->w;sh=src->h;sx=0;sy=0;}
  else {sw=srcrect->w;sh=srcrect->h;sx=srcrect->x;sy=srcrect->y;}
  uint8_t *dp=(uint8_t*)dst->pixels;
  uint8_t *sp=(uint8_t*)src->pixels;//是uint8
  //printf("dp:%d,sp:%d\n",strlen(dp),strlen(sp));
  for(int i = 0;i < sh;i++){
    memcpy(dp+ww*((i+dy)*(dst->w)+dx),sp+ww*((i+sy)*(src->w)+sx),ww*sw);
    //printf("%d\n",flag);
  }
}
